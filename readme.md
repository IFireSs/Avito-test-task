...
- Из задания убрали админский токен, но для деактивации пользователей его отсутствие выглядит странно, поэтому в качестве допущения оставил его опциональным в виде переменной окружения ADMIN_TOKEN. Если не задан - не требуется. дефолтный - avito-internship (реализация в виде bearer токена)
